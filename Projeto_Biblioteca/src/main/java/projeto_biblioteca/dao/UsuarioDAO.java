...
public class UsuarioDAO {
	...
	public String update(Usuario user, String emailOriginal) {
		
		Optional<Usuario> optional;
		
		try {
			// pega no banco o usuário que deve ser editado
			optional = getUserByEmail(emailOriginal);
		} catch(Exception e) {
			throw new RuntimeException("Erro durante busca por usuario no banco de dados | ", e);
		}
		
		if(optional.get() == null) {
			throw new RuntimeException("Usuario não foi encontrado no BD");
		}
		
		String sql = "UPDATE usuarios SET nome = ?, email = ?, senha = ?, telefone = ?, tipo_usuario = ? where id = ?;";
		...
		try(Connection conn = dataSource.getConnection(); 
			...
			ps.setString(6, String.valueOf(optional.get().getId()));
			
			ps.executeUpdate();
		}catch (SQLException e) {
			throw new RuntimeException("Erro durante o update no BD", e);
		}
	...
	public Optional<Usuario> getUserByEmailAndPassword(String email, String senha) {
		...
		String sql = "select id,email,nome,tipo_usuario,telefone from usuarios where email='"+email+"' and senha='"+senha+"'";
